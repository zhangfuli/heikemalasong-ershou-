...
angular.module('welcomePageApp')
  .controller('AboutCtrl', function ($scope,$routeParams,$http,$route) {
    ...
    $http.get("http://heike.y1code.cn:8080/second/review/getreview?publishid="+$routeParams.id).then(function(respond){
      $scope.says=respond.data;
    });
    $scope.pushon=function(){
      var sayword=$scope.sayword;
      $http.get("http://heike.y1code.cn:8080/second/review/doreview?publishid="+$routeParams.id+"&detail="+sayword);
      $route.reload();
    }
